# Warpline tools — exact input/output
...
`resolved|unresolved|unknown` — warpline's local honesty signal, NOT a lineage claim.
...
snapshot exists. `data.next_actions.filigree[]` holds *candidate* work — warpline
files nothing.
